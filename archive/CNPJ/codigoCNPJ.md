...
# ─── COSMOS ───────────────────────────────────────────
from cosmos import  ProjectConfig, ProfileConfig, DbtTaskGroup, ExecutionConfig
from cosmos.profiles import GoogleCloudServiceAccountDictProfileMapping # BigQueryUserPasswordProfileMapping
...
execution_config = ExecutionConfig(
    dbt_executable_path="/opt/airflow/dbt_venv/bin/dbt"  # Seu virtual environment
)

profile_config = ProfileConfig(
    profile_name="dbt-cnpj-project",
    target_name="dev",
...
##OPERATOR - DBT no Airflow
# Service Account do Kubernetes configurada com Workload Identity
# Esta SA do K8s deve estar vinculada a uma SA do GCP com as permissões necessárias
K8S_SERVICE_ACCOUNT_NAME = "airflow-worker" # Nome da Service Account do Kubernetes
K8S_NAMESPACE = "airflow" # Namespace onde o pod será executado
########################################
...
def cnpj2026():
    ...
    def check_site_url(pa: str, **context):
        ...
        if qtd_arquivos < 10:
            raise Exception(f"Dados insuficientes para {pa}: apenas {qtd_arquivos} arquivos encontrados.")
...
# ─── RUN DBT ────────────────────
    ...
    dbt_run_task = DbtTaskGroup(
        group_id="dbt_run_medalhao",
        project_config=ProjectConfig(
            dbt_project_path=(dbt_root_path / "dbt-cnpj-project").as_posix(),
            manifest_path=(dbt_root_path / "dbt-cnpj-project/target/manifest.json").as_posix()
        ),
